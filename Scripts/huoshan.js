...
async function sign_in(){
 return new Promise((resolve) => {
    let sign_inurl = {
   		url: hsurl.slice(0,50)+'task_system/'+str.slice(50),
   ...
   $.post(sign_inurl,async(error, response, data) =>{
    try{
        ...
        console.log('🎈'+result.msg+'!!'+result.data.TopContent+'\n')
        message += '🎈'+result.msg+'!!'+result.data.TopContent+'\n'
        }else{
        message += '👀'+result.msg+"\n"
        }
        }catch(e) {
 ...
 return new Promise((resolve) => {
   ...
   $.get(get_stepurl,async(error, response, data) =>{
    try{
        ...
        console.log('🎈'+result.message+'!! '+result.step_counts+'\n')
        now_step = result.step_counts
        message += '🎈'+result.message+'!! 当前步数：'+result.step_counts+'\n'
        }else{
...
let feedheader = hsheader.replace("{",`"{x-common-params-v2": "${hsurl}"`)
 return new Promise((resolve) => {
    let hotsoonfeed_url = {
...
let playheader = hsheader.replace("{",`"{x-common-params-v2": "${hsurl}"`)
	for(let i = 0;i <= 4;i++){
		let item_id_inv = item_id[i]
$.log(item_id_inv)
 return new Promise((resolve) => {
    let play_video_url = {
     url: 'https://api3-normal-c-lf.huoshan.com/hotsoon/item/reaction/_play',
    	headers: playheader, 	
     body: hsbody
}
   $.post(play_video_url,async(error, response, data) =>{
    try{
        ...
        message += '🎈视频播放成功！play_count=：'+result.data.play_count+'\n'
}
        }catch(e) {
          $.logErr(e, response);
      } finally {
 ...
 return new Promise((resolve) => {
   ...
   $.post(video_rewards_url,async(error, response, data) =>{
    try{
        ...
        if(logs)$.log(data)
        if(result.status_code == 0){
	    let token = result.data.next_token.replace("=","")
	    let newhsbody = hsbody.replace(/WJ.*?(?=%3D)/,`${token}`)
         $.setdata(newhsbody,`hsbody${status}`)
         $.log(newhsbody)
        let coins = result.data.flame_amount
        console.log(`🎈第${i}次获得火苗成功：`+coins+'\n')
        coins += coins
        message = '🎈获得火苗成功：'+coins+'\n'
        }else{
        console.log('👀'+'我也不知道\n')
        //message += '👀'+"我也不知道\n"
        }
